...
MCMC_function_GP <- function(sim_ind, hess, delta, stepsize, log_posteriors,
  ...
  alpha <- dnorm(beta_share[sim+1,1], 0, 5, log = T) - 
    dnorm(beta_share[sim,1], 0, 5, log = T) +
  ...
  alpha <-
    sum(test_gev_2) -
   sum(test_gev) + dnorm(beta_share[sim+1,3], 0, 5, log = T) -
    dnorm(beta_share[sim,3], 0, 5, log = T)
  ...
  if (log( u ) >alpha){
    ind_share[sim,3] <<- 0
    beta_share[sim+1,3] <- beta_share[sim,3] } else {ind_share[sim,3] <<- 1}
 ...
 # }
  ...
  if (!is.null(fe_mu)){
    ...
    alpha <- 
      sum(test_gev_2) -
      sum(test_gev)  + 
      prior_normal(x=beta_share[sim+1,5], mu=mu, sigma=sigma) -
      prior_normal(x=beta_share[sim,5], mu=mu, sigma=sigma) 
