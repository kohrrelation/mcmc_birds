...
g_func <- function(theta_1, theta_3,x,theta_2){
  (theta_2)*( 1/(1+exp(-x*(theta_1) ))^(1/theta_3) )
}
g_d_func <- function(theta_1, theta_3,x,theta_2){
  (theta_2) * ((1 + exp(-x * (theta_1)))^((1/theta_3) - 1) * ((1/theta_3) * 
                                                                (exp(-x * (theta_1)) * (theta_1)))/((1 + exp(-x * (theta_1)))^(1/theta_3))^2)
}
g_d_theta2_func <- function(theta_1, theta_3,x,theta_2){
  (1/(1 + exp(-x * (theta_1)))^(1/theta_3))
}
...
delta=c(0.001, 0.01, 0.005, 0.01, 0.01)
# hyperparam1,2,          
stepsize <- c(0.15, 0.3)
...
stepsize_share <- c(0.001, NA, 0.001, 0.001, 0.1)
...
for (sim in 1:(B-1)){
  ...
  if (adaptive==TRUE){
    if (sim%%300==0){
      ...
      which.low <- which(apply(ind_latent_gp[(sim-250):(sim-1),], 2, mean)<0.7)
      which.high <- which(apply(ind_latent_gp[(sim-250):(sim-1),], 2, mean)>0.95)
      delta[which.low] <- 0.5*delta[which.low]
      delta[which.high] <- 2*delta[which.high]
  
      if (mean(ind_fe[(sim-250):(sim-1),1])<0.7){
        stepsize_fe[1] <- 0.5*stepsize_fe[1]
      } else if (mean(ind_fe[(sim-250):(sim-1),1])>0.95){
        stepsize_fe[1] <- 2*stepsize_fe[1]
      }
  
      which.low <- which(apply(ind_intercept[(sim-250):(sim-1),], 2, mean)<0.25)
      which.high <- which(apply(ind_intercept[(sim-250):(sim-1),], 2, mean)>0.55)
      stepsize_intercept[which.low] <- 0.5*stepsize_intercept[which.low]
      stepsize_intercept[which.high] <- 2*stepsize_intercept[which.high]
  
      which.low <- which(apply(ind_fe[(sim-250):(sim-1),-1], 2, mean)<0.25)
      which.high <- which(apply(ind_fe[(sim-250):(sim-1),-1], 2, mean)>0.55)
      stepsize_fe[c(2:length(stepsize_fe))[which.low] ] <- 0.5*stepsize_fe[c(2:length(stepsize_fe))[which.low] ]
      stepsize_fe[c(2:length(stepsize_fe))[which.high] ] <- 2*stepsize_fe[c(2:length(stepsize_fe))[which.high] ]
  
      which.low <- which(apply(ind_share[(sim-250):(sim-1),], 2, mean)<0.25)
      which.high <- which(apply(ind_share[(sim-250):(sim-1),], 2, mean)>0.55)
      stepsize_share[which.low] <- 0.5*stepsize_share[which.low]
      stepsize_intercept[which.high] <- 2*stepsize_share[which.high]
  ...
  if (sim%%2000==0 | sim==10 | sim==500 | sim==1000){
    #### store
...
# burn_in <- 1
# 
# plot(trans_func_inv( g_func(beta_share[burn_in:(sim),1], beta_share[burn_in:(sim),2],2,
#              beta_mu[burn_in:(sim),]) ) , type='l', xlab='iteration', ylab='g_func_at_2')
