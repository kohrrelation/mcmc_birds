...
beta_xi[1,] <- -0.3
beta_sigma[1,] <- -1
beta_mu[1,] <- mean(dat.mat, na.rm=TRUE)
beta_pp[1,] <- log(50/size_A)
beta_pp_2[1,] <- log(mean(dat_bbs, na.rm=TRUE)) # 
beta_bin[1,] <- log(-log(1-mean( apply(n_s_i/n_s, 1, function(x) mean(x,na.rm=TRUE)) )))
...
# sharing 
beta_share[1,1] <- 0 # start with no sharing
beta_share[1,2] <- theta_3 # this is fixed in the MCMC
beta_share[1,3] <- 0 # start with no sharing first
beta_share[1,4] <- 1 # this is fixed in the MCMC
beta_share[1,3] <- 0
beta_share[1,5] <- -1

fe_mu[1,] <- 0.1
fe_bin[1,] <- -1 # should always start from negative values 

fe_pp <- matrix(nrow= B, ncol=(n.year ))
fe_pp[1,] <- rep(0, n.year)
